...
- `src/pages/[category]/index.astro` renders parent landing pages such as `/getting-started`
...
- preserves the parent section in the URL
- keeps breadcrumbs predictable
- makes sidebar context clearer

Sub-category pages use the same nested shape. A category like `compass-docs` is available at `/getting-started/compass-docs`, not `/compass-docs`. Compass does not generate redirects for one-segment sub-category URLs by default.
