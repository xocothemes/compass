...
During `npm run dev`, the site UI still works, but the generated Pagefind bundle does not exist yet. Compass keeps the Pagefind client disabled in dev mode so the browser does not try to load missing generated search files.
